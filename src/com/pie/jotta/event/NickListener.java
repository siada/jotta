...
public class NickListener implements MessageListener {

	public void recieve(IRCMessage message) {
		if(message.getCommand().equals("NICK")) {
			Logger.getInstance().log(message.getSender()+" is now known as "+ message.getMessage());
			IgnoreManager.changeName(message.getSender(), message.getMessage());
			IRCMethods.changeLastFmNick(message.getSender(), message.getMessage());
		}
	}
